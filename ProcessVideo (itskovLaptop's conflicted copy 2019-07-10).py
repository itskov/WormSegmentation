...
def splitBatch(batchData, bins):
    ...
    batchData = np.rollaxis(np.rollaxis(batchData, axis=2), axis=2)
    batchDataSize = batchData.shape
    ...
    splittedBatch = np.reshape(colSplit,
                               (int(batchDataSize[0] / bins), int(batchDataSize[1] / bins), batchDataSize[2] * bins**2))

    splittedBatch = np.rollaxis(splittedBatch, axis=2)
...
def mergeBatch(batchData, bins):
    ...
    reshaedSplittedData = np.rollaxis(np.rollaxis(reshaedSplittedData, axis=4), axis=3)


    fullData = np.reshape(reshaedSplittedData, (s[0] * bins, s[1] * bins, int(s[2] /  bins ** 2)))

    fullData = np.rollaxis(fullData, axis=2)
...
def main():
    ## DEBUG
    im = data.coins()
    mergeBatch(splitBatch(np.reshape(im, (1, im.shape[0], im.shape[1])),3), 3)
